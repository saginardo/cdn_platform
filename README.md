...
- Go control plane with SQLite metadata, Argon2id password login, TOTP, one-time recovery codes, CSRF protection, audit records, and a compact management UI with dedicated node/site detail pages, per-node 24-hour cache outcomes, reported cache disk usage, and confirmation-protected deletion workflows.
