...
proxy_cache_path /var/cache/cdn-platform levels=1:2 keys_zone=cdn_cache:100m inactive=7d max_size={{.CacheMaxSize}} use_temp_path=off;
...
server {
    ...
    ssl_certificate /etc/cdn-platform/certs/{{.ID}}.crt;
    ssl_certificate_key /etc/cdn-platform/certs/{{.ID}}.key;
    ssl_protocols TLSv1.2 TLSv1.3;
    ssl_session_cache shared:TLS:10m;

    set $cdn_site_id "{{.ID}}";
    access_log /var/log/cdn-platform/access.json cdn_json;
...
func Render(sites []domain.Site) (string, error) {
	...
	for _, site := range sites {
		...
		item := renderedSite{
			...
			GRPC: domain.IsGRPCScheme(primary.Scheme), Passthrough: site.Passthrough,
		}
	...
	if err := template.Execute(&out, renderInput{Sites: items, CacheMaxSize: defaultCacheMaxSize}); err != nil {
		return "", err
	}
